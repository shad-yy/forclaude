...
test.describe('Navigation — No Dead Ends', () => {
    ...
    test('Buy Now buttons go to /buy not /free-trial', async ({ page }) => {
        await page.goto(BASE)
    ...
    test('Free Trial buttons only go to /free-trial', async ({ page }) => {
        await page.goto(BASE)
...
test.describe('Conversion Funnel — Forms and CTAs', () => {
    ...
    test('/buy page loads and has a functional form', async ({ page }) => {
    ...
    test('pricing page shows all 4 plans with correct prices', async ({ page }) => {
        await page.goto(`${BASE}/pricing`)

        const prices = ['£12', '£24', '£36', '£54']
        const pageText = await page.textContent('main, body')

        for (const price of prices) {
            expect(
                pageText,
                `Price ${price} not found on /pricing`
            ).toContain(price)
        }

        // Monthly/Annual toggle should NOT exist
        const toggle = await page.$('text=Annual, button:has-text("Annual")')
        // If it exists, it must be removed as per our fix
        // This test documents the expected state
        console.log('Annual toggle present:', toggle !== null)
    })
})
...
test.describe('Mobile — iOS Safari Critical Issues', () => {
    ...
    test('mobile sticky CTA bar is present on pricing page', async ({ page }) => {
        await page.goto(`${BASE}/pricing`)
...
test.describe('Content — No Outdated or Wrong Data', () => {
    ...
    test('no "2025" year in setup page titles', async ({ request }) => {
    ...
    test('channel counts are consistent (15k for live, 230k for total)', async ({ page }) => {
        await page.goto(BASE)

        const pageText = await page.textContent('body')

        // 230,000 should appear (total channels including VOD)
        expect(pageText).toContain('230,000')

        // Log instances of each number for review
        const instances230k = (pageText?.match(/230,000/g) || []).length
        const instances15k = (pageText?.match(/15,000/g) || []).length

        console.log(`230,000+ mentions on homepage: ${instances230k}`)
        console.log(`15,000+ mentions on homepage: ${instances15k}`)
    })
