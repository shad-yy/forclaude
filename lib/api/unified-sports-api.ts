...
class UnifiedSportsAPI {
  ...
  getSeasonString(year?: number): string {
    ...
    // If we're in the second half (Aug-Dec), we're in the current season
    let seasonStart: number
    let seasonEnd: number

    if (month < 7) {
      // Jan-Jul: we're in season (currentYear-1)-(currentYear)
      seasonStart = currentYear - 1
      seasonEnd = currentYear
    } else {
      // Aug-Dec: we're in season currentYear-(currentYear+1)
