...
async function LeaguesContent({ onLeagueClick }: { onLeagueClick: (league: UnifiedLeague) => void }) {
  try {
    ...
    const allLeagues = await unifiedSportsAPI.getLeagues()

    // Group leagues by sport
    const leaguesBySport = new Map<string, typeof allLeagues>()
    ...
    // Get top leagues (Premier League, La Liga, Serie A, Bundesliga, Ligue 1)
    const topLeagueIds = ["4328", "4335", "4332", "4331", "4334"]
    const topLeagues = allLeagues.filter((league) => topLeagueIds.includes(league.id))
        ...
        <Card className="bg-gradient-to-br from-blue-900/20 to-blue-700/20 border-blue-500/30">
          <CardContent className="p-6 text-center">
            <Trophy className="w-8 h-8 mx-auto mb-2 text-blue-400" />
              <div className="text-2xl font-bold text-white mb-1">{allLeagues.length}+</div>
